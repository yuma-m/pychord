...
class TestChordCreations(unittest.TestCase):
    ...
    def test_inversion(self, chord_str, expected_root, expected_quality, expected_components):
        c = Chord(chord_str)
        self.assertEqual(expected_root, c.root)
        self.assertEqual(expected_quality, c.quality.quality)
...
class TestChordFromNoteIndex(unittest.TestCase):
    ...
    def test_diatonic_from_note_index(self, note, quality, diatonic, scale, expected_chord):
        chord = Chord.from_note_index(note=note, quality=quality, diatonic=diatonic, scale=scale)
        self.assertEqual(Chord(expected_chord), chord)
